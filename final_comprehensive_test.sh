...
# Final Comprehensive API Testing Script for FinGaurd Application
...
echo -e "${BLUE}🚀 Final Comprehensive API Testing for FinGaurd Application${NC}"
echo "=========================================================="
echo ""
...
echo -e "${YELLOW}Test 1: Duplicate Email Registration${NC}"
RESPONSE=$(make_request "POST" "$BASE_URL/api/auth/signup" "$USER_DATA")
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
...
if [ "$HTTP_CODE" = "409" ]; then
    ...
    log_test "Duplicate Email Registration" "FAIL" "HTTP $HTTP_CODE"
fi
...
echo -e "${YELLOW}Test 3: Weak Password${NC}"
WEAK_PASSWORD_DATA='{
    "username": "testuser3",
...
echo -e "${YELLOW}Test 5: Valid Transaction Creation${NC}"
TRANSACTION_DATA='{
    "amount": 150.75,
...
echo -e "${YELLOW}Test 6: Invalid Amount (Negative)${NC}"
INVALID_AMOUNT_DATA='{
    "amount": -50.00,
    "transactionType": "EXPENSE",
    "category": "Shopping",
...
echo -e "${YELLOW}Test 7: Invalid Amount (Zero)${NC}"
ZERO_AMOUNT_DATA='{
    "amount": 0,
    "transactionType": "EXPENSE",
    "category": "Shopping",
...
# Test 9: Very Large Amount
echo -e "${YELLOW}Test 9: Very Large Amount${NC}"
LARGE_AMOUNT_DATA='{
    "amount": 999999999.99,
...
# Test 10: Long Description (501 characters)
echo -e "${YELLOW}Test 10: Long Description Validation${NC}"
...
echo -e "${YELLOW}Test 11: Get Transactions${NC}"
...
# Test 12: Invalid Pagination (Negative Page)
echo -e "${YELLOW}Test 12: Invalid Pagination (Negative Page)${NC}"
...
# Test 13: Excessive Page Size
echo -e "${YELLOW}Test 13: Excessive Page Size${NC}"
...
if [ -n "$TRANSACTION_ID" ]; then
    echo -e "${YELLOW}Test 14: Get Specific Transaction${NC}"
...
# Test 15: Get Transaction Statistics
echo -e "${YELLOW}Test 15: Get Transaction Statistics${NC}"
...
if [ -n "$TRANSACTION_ID" ]; then
    echo -e "${YELLOW}Test 16: Delete Transaction${NC}"
...
echo -e "${YELLOW}Test 17: Access Without Authentication${NC}"
...
# Test 18: Access With Invalid Token
echo -e "${YELLOW}Test 18: Access With Invalid Token${NC}"
...
# Test 19: Rapid Sequential Requests
echo -e "${YELLOW}Test 19: Rapid Sequential Requests${NC}"
RAPID_SUCCESS=0
for i in {1..5}; do
    RAPID_DATA='{
        "amount": '$((i * 10))'.50,
...
# Test 20: Large JSON Payload (10KB description)
echo -e "${YELLOW}Test 20: Large JSON Payload${NC}"
LARGE_PAYLOAD_DESC=$(printf 'X%.0s' {1..10000})
LARGE_PAYLOAD='{
    "amount": 100.00,
    "transactionType": "EXPENSE",
    "category": "Large Payload Test",
    "description": "'$LARGE_PAYLOAD_DESC'"
}'
...
if [ $FAILED -eq 0 ]; then
    ...
    echo -e "${GREEN}✅ Transaction Management: Working perfectly${NC}"
    echo -e "${GREEN}✅ Security & Authorization: Working perfectly${NC}"
    echo -e "${GREEN}✅ Input Validation: Working perfectly${NC}"
    echo -e "${GREEN}✅ Edge Cases & Stress Testing: Working perfectly${NC}"
    echo -e "${GREEN}✅ Error Handling: Working perfectly${NC}"
    echo ""
    echo -e "${BLUE}🚀 The FinGaurd application is PRODUCTION-READY and BULLETPROOF! 🚀${NC}"
    exit 0
else
    echo ""
